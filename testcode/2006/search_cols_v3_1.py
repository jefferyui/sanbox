...
def rule_to_str(rule):
    ...
    return ", ".join([f"{f}>={th}" for f,th in rule])

def build_grid_hybrid(X, steps, n_quantiles=5):
    
    grid = {}
    
    for f in X.columns:
        step = steps.get(f, None)
        
        values = []
        
        # 1️⃣ fixed step（精度控制）
        if step is not None:
            vmin, vmax = X[f].min(), X[f].max()
            values += list(np.arange(vmin, vmax, step))
            print(f"X:{X}, f:{f}, step:{step}, values:{values} , vmin:{vmin}, vmax:{vmax}")
        # 2️⃣ quantile（避免漏重要區間）
        qs = np.linspace(0.1, 0.9, n_quantiles)
        values += list(np.quantile(X[f], qs))
        print(f"X:{X}, f:{f}, values:{values}, np.quantile(X[f], qs):{np.quantile(X[f], qs)}")
        # 去重 + 排序
        grid[f] = np.unique(np.round(values, 3))
...
def beam_search_df(X, y, beam_width=5, max_depth=3):
    ...
    grid = build_grid_hybrid(X, steps)
